...
<%
   // String USERID    = common.setNullToString((String)session.getAttribute("SESSTAFFID"));
    ...
    String CNCODE       = common.setNullToString((String) session.getAttribute("SESCNCODE"));
    String CNCODE1      = common.setNullToString((String) session.getAttribute("SESCNCODE1"));
    String CLASS     	= common.setNullToString((String) session.getAttribute("CLASS"));
    	if(CLASS.equals(""))
    		CLASS    = common.setNullToString(request.getParameter("CLASS"));
    ...
    try {
		...
		if(!error)
		{
			try
		    {
		        ...
		        SQL2 = "SELECT ACCODE,CNCODE,PRINCIPLE,ORCCODE,POLNO,ENDORSE_NO,STATUS,PREVPOL,REPLACECN,MASTERIND,NEW_IC_NO,OLD_IC_NO,BUSINESS_NO,CONTACT_TYPE FROM TB_FWHSCN WHERE UKEY = '"+CNCODE+"' WITH UR";
			    ...
			    //---GST ENDORSEMENT--
		        SQL = "SELECT CNSTATUS FROM TB_TRANSACTION WHERE IDNO='"+CNCODE+"'";
				...
				String temp_CNCODE 	  = CNCODE;
				if(PM_ENDORSE.equals("Y"))
					temp_CNCODE = PRINCIPLE+REPLACECN;
				
				SQL = "SELECT * FROM TB_GST_CN WHERE UKEY = '"+temp_CNCODE+"' WITH UR";
				DB_Contact.executeQuery(SQL);
			...
			boolean howdenAgent 		= headerFooterChange.equals("Y");
				
			if(GST_STATUS.equals("SAVED"))
		    ...
		    // Get Stamp Fees from DB to display in PDF 
	        SQL2 = "SELECT SUMINS,GPREM,STAXAMT,STAMPDUTY,NETPREM,COMMAMT,TOTPREM, STAMP_FEES FROM TB_FWHSSCH WHERE UKEY2='"+CNCODE+"' WITH UR";
			DB_Template.makeConnection();
	        DB_Template.executeQuery(SQL2);
			...
			fileURL += "/template/"+SRC_URL+"?BUTTONIND="+BUTTONIND+"&option=print&POLTYPE2="+POLTYPE2+"&CLASS="+CLASS;
			...
			{	
				String CLASS_DESCP = "";
				SQL = "SELECT CLASS,PRINCIPLE FROM "+table+" WHERE UKEY = '"+CNCODE+"' WITH UR";
		        DB_Template.makeConnection();
		        DB_Template.executeQuery(SQL);
		        while(DB_Template.getNextQuery())
		        {
            ...
            if (LANGUAGE.equalsIgnoreCase("chinese")){
                RP_html2pdf.generateHtml(SESUSERID + "-FWHS-" + CNCODE + ".pdf",HTML,"chinese","PORTRAIT","","<p align='center'>Page %%Currentpagenumber%</p>");
            }else
            { 
				String size3 = "7";
				String size4 = "5";
				...
				if(SRC_URL.equals("pop_cn_fwhs_end_preview.jsp"))
				{
					...
					if(testHTML.length()>0){ 			
						...
						else
						{
							if(idxHTML3 > 0)
							{
								...
								//yxng 200 length is to make sure there is html content
								if(testHTML.length()>200){  
									alHTML.add("");
									alHTML.add(testHTML);
								}
							}
					...
					for(int len=0; len < alHTML.size(); len++){
	                    ...
	                    else
	                    {
							if(len==0){
								if(WITHOUTLOGO.equals("Y")){
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height,"70","80");
								}else{
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height2,"65","30");
								}
							}else if(len==2){
								if(WITHOUTLOGO.equals("Y")){
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT","","",footer3, footer3,"10","90","90");
								}else{
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT","","",footer3, footer3,"10","65","30");
								}
							}else
							{
							
								if(WITHOUTLOGO.equals("Y")){	
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height,"90","90");
								}else{
									RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + "pg"+len+".pdf",curr,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height2,"65","30");
								}
							}
					...
					PdfReader readerPage = new PdfReader(TEMP_PATH + "/" + SESUSERID + "-FWHS-" + CNCODE + "pg0.pdf");    
					Rectangle pagesize = readerPage.getPageSize(1);    
						
				    Document document = new Document(pagesize);
				    PdfWriter Pdfwriter = PdfWriter.getInstance(document, new FileOutputStream(TEMP_PATH + "/" + SESUSERID + "-FWHS-" + CNCODE + ".pdf"));
				    document.open();
					...
					for(int comb=0 ; comb < alHTML.size(); comb++){
						...
						{
							PdfReader readerInner = new PdfReader(TEMP_PATH + "/" + SESUSERID + "-FWHS-" + CNCODE + "pg"+comb+".pdf");
					...
					for(int remove = 0 ; remove < alHTML.size(); remove ++){
						...
						{
							File tempfile = new File(TEMP_PATH + "/" + SESUSERID + "-FWHS-" + CNCODE + "pg"+remove+".pdf");
							if (tempfile.exists()){
								tempfile.delete();
							}
						}
					...
					//removed for duplicate header display
/* 				
					if(WITHOUTLOGO.equals("Y"))
			    		RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + ".pdf",HTML,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height,"90","90");
			    	else
			    		RP_html2pdf.generateHtml_custom_footer2(SESUSERID + "-FWHS-" + CNCODE + ".pdf",HTML,"english","PORTRAIT",headerHTML,headerHTML2,footer2, footer1,logo_height2,"65","30");
*/		
				...
				response.sendRedirect("pop_quoFWHS_generate_preview.jsp?STATUS="+STATUS+"&CNCODE="+CNCODE+"&CLASS=FWHS&BUTTONIND="+BUTTONIND+"&QUICK_IND="+QUICK_IND+"&FWCMSREF="+FWCMSREF+"&FWCS_CONV_IND="+FWCS_CONV_IND+"&FWIG_CONV_IND="+FWIG_CONV_IND); 		  
    } 
    catch (Exception e) 
    {
	    e.printStackTrace();
