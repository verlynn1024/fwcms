...
--%><jsp:useBean id="common" scope="page" class="com.rexit.easc.common" /><%--
     ...
     IMPORTANT NOTICE document template - the online-portal port of the
     legacy include pop_incl_f2.jsp, and structured like it: the same
     request parameters (checkdigit, check_ind), the same e-ASC checkdigit
     line, and the same check_ind branch tables. In the legacy EASC app the
     Important Notice is NOT a static PDF - pop_cn_FWIG_SCH_preview.jsp
     includes pop_incl_f2.jsp with check_ind="Y" (FWIG) and
     pop_cn_fwhs_preview.jsp with check_ind="H" (FWHS), rasterised to PDF
     by the html2pdf generator. The mandatory-appendix step of
     gen_fwcms_pdf.jsp loops back to this template (TYPE=GRAB, check_ind
     "Y"/"H" by DOC, checkdigit computed as in the legacy preview) instead
     of merging an Important_Notice.pdf, so the notice is generated from
     the JSP, not a file - for BOTH FWIG_SCH and FWHS_SCH.

     The check_ind "Y" and "H" tables are character-for-character copies of
     pop_incl_f2.jsp's branches (20260422 update: FMOS + BNMLINK two-column
     point 5, PIDM point 6): identical notice text, differing only by the
     H branch's tablefitpage="on", exactly as in the legacy include. An
     empty / unrecognised check_ind falls back to the "Y" table so a bare
     loopback still renders the FWIG notice.

     The notice carries no per-policy data, so there is no DAO read - only
     the GRAB / session guards, matching the other templates. Font sizes
     are emitted quoted (size="1", size="2") so
...
--%><%
	...
	/* pop_incl_f2.jsp parameters, forwarded by gen_fwcms_pdf.jsp */
	...
	System.out.println("[FWCMSPRINT] UUID=" + UUID + " DOC=IMPORTANT_NOTICE stage=template-entry - "
		...
		+ " TYPE=[" + TYPE_PARAM + "] check_ind=[" + sCHECK_IND + "] checkdigit=[" + sCheckdigit
	...
	/* only the FWIG ("Y") and FWHS ("H") branches of pop_incl_f2.jsp exist
	   in the portal - default anything else to the FWIG notice */
	if (!sCHECK_IND.equals("Y") && !sCHECK_IND.equals("H"))
	{
		sCHECK_IND = "Y";
	}

	System.out.println("[FWCMSPRINT] UUID=" + UUID + " DOC=IMPORTANT_NOTICE stage=template-render - "
		+ "rendering important notice HTML (check_ind=" + sCHECK_IND + ")");
%>
<html>
<br/><br/>
